# 🤖 CryptoOracle: AI 驱动的量化交易终端 (v3.2 Async Core)

> **"像华尔街专业人士一样思考，像算法机器人一样执行。"**

CryptoOracle Plus是一个集成 **DeepSeek-V3** 大模型与 **CCXT** 交易框架的下一代量化交易系统。
v3.2 版本采用了全新的 **AsyncIO 异步内核** 与 **组件化架构**，实现了从单体脚本到企业级工程的蜕变。
...
*   **智能资金管理 (Smart Fund Sharing)**:
    *   **稳健模式**: 严格遵守单币种配额，风险隔离。
    *   **弹性激进模式**: 在高确信度 (HIGH Confidence) 机会下，自动突破配额限制，调用账户闲置资金（最高 90%）以最大化收益。
...
### 7. 💻 仪表盘表格视图 (Dashboard Table View) **[v3.1.3 New]**
*   **多币种监控**: 彻底重构了控制台输出，采用 **结构化表格** 替代了传统的滚动日志。
*   **即时信息**: 
    *   `SYMBOL`: 币种名称
    *   `PRICE`: 实时价格 (千分位显示)
    *   `24H%`: 24小时涨跌幅 (带 🟢/🔴 图标)
    ...
    *   `CONF`: AI 置信度 (🔥🔥/⚡/💤)
    *   `SUMMARY`: AI 决策理由摘要
*   **优势**: 支持同时监控 50+ 个交易对而不会刷屏，所有币种状态一目了然，宛如专业的彭博终端。
...
```bash
# 1. 克隆代码
git clone https://github.com/your-repo/OKXBot_Plus.git
cd OKXBot_Plus
...
# 编辑 config.json，填入您的 API Key 和策略参数

# 4. 设置环境变量 (可选，用于 API Key)
cp .env.example .env
...
在 `config.json` 中设置 `"enable_aggressive_mode": true`。
- **开启后**: 当 AI 信心极高 (HIGH) 时，允许突破基础配额，调用闲置资金进行重仓交易。
- **日志**: 启动时依然显示“锁定本金”，但在交易时会根据信号动态调整。

### 日志系统
- 日志文件位于 `log/` 目录下。
- 文件名格式: `trading_bot_YYYYMMDD_HHMMSS.log`。
- 系统自动保留最近 30 个日志文件，防止磁盘占满。
