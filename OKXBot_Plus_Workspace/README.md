...
## 7. 📦 打包与部署 (Packaging)
本项目支持 **Docker** 容器化部署与 **Windows EXE** 打包。
详细指南请参阅：[📦 打包与部署指南 (Packaging Guide)](doc/PACKAGING_GUIDE.md)

*   **Docker**: 提供 `Dockerfile` 与 `docker-compose.yml`，一键启动。
*   **Windows**: 提供 `build_exe.bat`，一键生成独立的 `.exe` 可执行文件。
