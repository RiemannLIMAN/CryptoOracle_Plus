...
if [ -d "data" ]; then
    ...
    rm -f data/bot_state.json 2>/dev/null
    # 强制执行零点校准 (Zero-Start)
    echo -e "${GREEN}✅ 已清理历史基准和状态文件，确保 Session PnL 归零${NC}"
fi
    ...
    if [ -n "$CONDA_PREFIX" ]; then
        ...
        if [ -n "$CUSTOM_VENV_NAME" ] && command -v conda &> /dev/null; then
            ...
            if [ -n "$CONDA_ENV_PATH" ] && [ -f "$CONDA_ENV_PATH/bin/python" ]; then
                ...
                echo -e "${YELLOW}⚠️ 未找到名称 '$CUSTOM_VENV_NAME' 的 Conda 环境，或无法解析其路径${NC}"
            fi
        fi
...
if ps -p $NEW_PID > /dev/null; then
    ...
    echo -e "💡 按 ${YELLOW}Ctrl+C${NC} 可退出日志查看 (机器人将继续在后台运行)"
    echo "--------------------------------------------------"
    # 使用 tail -f 实时跟踪日志，从最后 50 行开始显示
    tail -n 50 -f "$STARTUP_LOG"
else
