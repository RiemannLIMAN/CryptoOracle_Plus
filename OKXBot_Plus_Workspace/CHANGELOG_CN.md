...
### 📈 移动止盈 (Trailing Stop)
- **看门狗升级 (Watchdog Upgrade)**:
  - 在看门狗的高频监控中新增了 **移动止盈** 逻辑。
  - **机制**:
    - **激活 (Activation)**: 当持仓浮盈达到配置阈值 (如 5%) 时，自动激活追踪模式。
    - **水位记录 (High Water Mark)**: 系统会实时记录持仓期间的最高盈利点。
    - **回撤触发 (Callback)**: 如果价格从最高点回撤超过配置幅度 (如 1%)，立即触发止盈。
  - **配置**: 新增 `trailing_stop_activation` 和 `trailing_stop_callback` 参数。
  - **收益**: 解决了“硬止盈”过早下车和“死拿”导致利润回吐的难题，让利润在暴涨行情中尽可能奔跑。

## [3.3.13] - 2025-12-27

### 🛡️ 风控增强 (Risk Enhancement)
- **看门狗止盈 (Watchdog Take Profit)**:
  - **背景**: 之前的看门狗 (Watchdog) 仅负责拦截亏损（硬止损），但无法在策略休眠期内捕捉暴涨利润。
  - **新功能**: 升级了 `run_safety_check`，新增了对 **硬止盈 (Hard Take Profit)** 的支持。
  - **逻辑**: 如果在休眠期内，浮盈超过配置的 `max_profit_rate` (如 +20%)，看门狗将立即触发 **HIGH** 信心平仓。
  - **收益**: 彻底消除了“双频轮询”架构下可能错失瞬间暴涨行情的风险，实现“既防暴跌，也抓暴涨”。

## [3.3.12] - 2025-12-27

### 🚀 架构升级 (Architecture Upgrade)
- **双频轮询架构 (Dual-Frequency Polling)**:
  - 为了完美平衡“降低交易频率”与“实时止损”的矛盾，系统升级为双频轮询模式。
  - **低频策略 (Strategy Loop)**: 默认 **30秒/60秒** 执行一次。负责 K 线分析、AI 决策和开仓。这有效过滤了市场噪音，防止震荡磨损。
  - **高频看门狗 (Watchdog Loop)**: 固定 **5秒** 执行一次。只负责获取当前价格并检查是否触发硬止损。不进行 AI 分析，不消耗 Token。
  - **效果**: 即使您将 AI 思考周期设置为 5分钟一次，机器人依然能在暴跌发生的 5秒内触发硬止损，彻底消除了长轮询带来的“止损盲区”风险。

## [3.3.11] - 2025-12-27

### 🛡️ 震荡磨损保护 (Anti-Whipsaw)
- **平仓拦截增强**:
  - 修复了“反手交易”时拦截逻辑的缺失。现在无论是 **Long -> Sell** 还是 **Short -> Buy**，只要 AI 信心不是 HIGH，都会进行微利检查。
- **防止震荡磨损 (Whipsaw Protection)**:
  - 新增了针对极短线震荡的保护逻辑。
  - **逻辑**: 如果持仓时间不足 **45秒** 且浮亏在 **-0.6%** 以内（属于正常波动范围），且 AI 信心不足 HIGH，系统将**强制拦截平仓**。
  - **目的**: 防止机器人在高频震荡行情中，因 AI 信号频繁跳变而反复“追涨杀跌”导致本金不断磨损。给交易留出一点“呼吸空间”。
...
- **术语区分 (Terminology)**:
  - 优化了日志和通知中的交易动词。
  - **合约模式**: 显示为 **"开多" (Open Long)**，以区别于现货。
  - **现货模式**: 保持为 **"买入" (Buy)**。
  - 消除了在合约交易中看到 "买入" 字眼时的语义混淆，使操作意图更精准。

## [3.3.9] - 2025-12-27

### 🛡️ 风控增强
- **稳健反手保护 (Safe Flip Guard)**:
  - 针对震荡行情优化了反手逻辑。
  - **旧逻辑**: 平仓后立即反手开空/做多（无论信心高低）。
  - **新逻辑**: 仅当 AI 信心为 **HIGH** 时才允许立即反手。如果信心为 **MEDIUM/LOW**，平仓后将**强制空仓观望**，等待下一次信号确认。
  - **效果**: 极大减少了在震荡市中因盲目反手导致的“双面耳光”磨损。
