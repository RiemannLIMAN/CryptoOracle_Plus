...
class DeepSeekAgent:
    ...
    def _get_role_prompt(self, volatility_status):
        if volatility_status == "HIGH_TREND":
            return "你是一位激进的趋势跟踪交易员。当前市场处于【单边剧烈波动】，ADX显示趋势极强。请紧咬趋势，果断追涨杀跌，不要轻易猜顶猜底。对于顺势机会（无论是做多还是做空），即使信心只有MEDIUM也请果断出手。"
        elif volatility_status == "HIGH_CHOPPY":
            return "你是一位冷静的避险交易员。当前市场处于【剧烈震荡】，波动大但无明显方向。请极度谨慎，优先选择观望，或在布林带极端位置做超短线反转（高抛低吸）。"
...
ADX (14): {adx_str} (趋势强度)"""
        ...
        2. **反手逻辑**：如果确信趋势已反转（如强劲的K线吞没或形态突破），请在建议平仓的同时，在 reason 中暗示反手开仓意图（Confidence设为HIGH）。
        3. **止损优先**：如果浮亏 > 3% 且形态崩坏，不要死扛，立即建议 SELL (平多) 或 BUY (平空)。
        4. **卖出风控 (关键)**：
