...
*   **🧩 动态自适应策略**: 告别死板参数，机器人会自动根据 K 线周期 (Timeframe) 调整分析窗口长度和波动率计算逻辑，在短线和中线之间游刃有余。
...
### 6.2 交易决策：AI 如何思考？
1.  **数据收集**: 机器人根据当前周期**动态获取历史 K 线** (如 1m 周期看 60 根，15m 周期看 32 根)，并计算 MACD, RSI, 布林带等指标。
...
A: 为了保持目录整洁，v3.4.4 采用了单日志文件模式 (`crypto_oracle.log`)，并会自动清理 3 天前的旧日志。
