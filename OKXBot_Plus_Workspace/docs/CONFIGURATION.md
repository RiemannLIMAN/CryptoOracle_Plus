...
*   **知其所以然**: 为什么建议 `10` 秒？
    ...
    *   **API 频率平衡**: 这是在“风控灵敏度”与“API 安全性”之间取得的最佳平衡点。
        *   *如果设为 1s*: 小资金账户多币种运行时，极易触发交易所的 `Rate Limit (429)`，导致账号被临时封禁。
        *   *10s 的保护*: 虽然在极端秒级插针下可能有 1-2% 的滑点损耗，但能确保系统长期稳定运行。
    *   **兜底机制**: 系统会自动在交易所端挂载 **硬止损单 (Hard Stop)**。硬止损由交易所服务器撮合，**不受 10s 循环延迟影响**。
*   **建议**: 10-20s。
...
### `sentiment_filter` (逆向情绪过滤)
*   **数据源**: 基于 Alternative.me 的恐惧与贪婪指数 (0-100)。
*   **`extreme_greed_threshold` (贪婪拦截)**: 
    *   **原理**: 当得分高于此值（如 75）时，禁止执行新的 `BUY` 指令。
    - **目的**: 防止在市场过热、散户疯狂入场时“追高接盘”。
*   **`extreme_fear_threshold` (恐慌拦截)**: 
    *   **原理**: 当得分低于此值（如 25）时，禁止执行新的 `SELL` 指令。
    - **目的**: 防止在市场极度恐慌、价格可能见底时“地板割肉”或在低位开空。
*   **建议**: 保持默认 (25 / 75)。
