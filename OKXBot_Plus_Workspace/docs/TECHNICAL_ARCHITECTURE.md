...
CryptoOracle (OKXBot Plus) 采用 **“三轨异步 (Triple-Track Async)”** 架构，旨在彻底解决“AI 深度思考耗时”与“高频风控实时性”之间的矛盾。

### 1.1 核心设计理念
*   **左脑 (Python)**: 负责精确计算、资金审计、毫秒级风控、高频止盈。
*   **右脑 (DeepSeek-V3)**: 负责模糊推理、形态识别、宏观趋势研判及仓位比例建议。

### 1.2 三轨并行机制
系统运行在全异步环境下，根据任务紧急程度分为三条独立轨道：

| 轨道 | 频率 | 职责 | 关键模块 |
| :--- | :--- | :--- | :--- |
| **轨道 A (战略层)** | 低频 (5m/15m) | 趋势分析、信号生成、仓位比例建议 | `AIStrategy`, `DeepSeek` |
| **轨道 B (执行层)** | 事件驱动 | 订单拆解、资产盘点、余额双重检查 | `OrderExecutor`, `TradeExecutor` |
| **轨道 C (战术层)** | **极高频 (10s)** | **[v3.9.6 核心]** 移动止盈(ATR)、分段止盈、每日利润锁定、账户级熔断 | `RiskManager`, `TrailingStop` |
