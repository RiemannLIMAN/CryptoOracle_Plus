...
### 🐛 Fixes
- **Auto-Fund Mode**: Corrected allocation logic. Now uses **Total Equity** instead of Available Balance as the base, preventing the "Shrinking Quota" bug.
- **Closing Logic**: Fixed a critical bug where closing positions (SELL) was blocked by "Insufficient Balance" checks. Closing logic now correctly ignores USDT quota limits.
- **Refactor**: Unified `get_account_equity` logic for accurate capital assessment.
...
- **Asset Initialization Display**:
  - Fixed an issue where the asset initialization table showed `N/A` for allocation and `0.00` for quota when running in "Auto Fund Mode" (no `initial_balance` configured).
  - It now correctly calculates and displays the theoretical quota based on the current account equity and allocation percentage (e.g., `50% | 500.00 U`).
