...
*   **智能资金回流 (Backflow Detection)**: 自动识别内部资产划转与外部充值的区别，确保卖出资产后资金能立即回流至可交易池。
...
*   **执行透明化**: 控制台看板新增 `EXECUTION` 状态列，明确显示每笔交易是 `DONE` (成交) 还是被 `QUOTA` (配额不足)、`MIN` (金额过小) 或 `PROFIT` (微利) 拦截。
*   **自动费率校准**: 启动时自动获取账户 VIP 等级对应的真实手续费率。
*   **诊断式报错**: 下单失败时，发送包含账户余额、缺口金额、AI 建议值的详细 Markdown 诊断报告。
*   **可视化战绩**: 内置 `plotter.py` 引擎，支持生成专业级 **PnL 资金曲线图** (Equity Curve) 和 **盈亏分布散点图**，复盘分析更直观。
*   **多渠道通知**: 支持 **Feishu (飞书)**、**DingTalk (钉钉)** 等实时推送。v3.1.15 全新升级了飞书卡片，支持动态彩色标题、Markdown 排版及资金变动展示（金额/余额/权益）。
